...
getPseudomatches <- function(col1, col2){
    matches_col1 <- as.character(sapply(col2, function (x){
        ...
        reverse_pseudomatch <- sapply(trimmed_col1, function (x) grepl(gsub("\\]","\\\\]", gsub("\\[","\\\\[",x)), trimmed_GO) )
...
pseudoMatches <- rbind(
    ...
    getPseudomatches(functool4, functool6),
...
```{r}
trimmed_GO <- functool4[1]
trimmed_col1 <- functool4
#unlist(strsplit(A, "; "))



delimited_matches <- sapply(unlist(strsplit(trimmed_GO, "; ")), function( x ) { grepl(format(x), tolower(trimmed_col1))})
forward_pseudomatch<- sapply(split(delimited_matches, seq(nrow(delimited_matches))), function ( x ) {"TRUE" %in% x})

# A - one term
# B - column of terms
#match <- function (A, B){
#    delimited_matches <- sapply(unlist(strsplit(A, "; ")), function( x ) { grepl(tolower(x), tolower(B), fixed = TRUE)})
#    sapply(split(delimited_matches, seq(nrow(delimited_matches))), function ( x ) {"TRUE" %in% x})
#}
forward_pseudomatch
reverse_pseudomatch <-sapply(trimmed_col1, function (x) {
...
```
```{r}
test1 <- c("arylesterase activity; carbonate dehydratase activity",
           "activation of MAPK activity; in utero embryonic development; endopeptidase inhibitor activity",
           "carbonate",
           "kidney",
           "kid",
           "development")
test2 <-  c("kidney development; morphogenesis of an epithelium; arylesterase activity; carbonate dehydratase activity",
           "endopeptidase; oxygen carrier activity; extracellular region; extracellular space; development",
           "arylesterase; kid",
           "utero",
           "activity",
           "ney")
# "arylesterase activity; carbonate dehydratase activity"   <->   "kidney development; morphogenesis of an epithelium; arylesterase activity; carbonate dehydratase activity"
# "arylesterase activity; carbonate dehydratase activity"   <-> "arylesterase; kid"
# "arylesterase activity; carbonate dehydratase activity"   <->  "activity"
# "activation of MAPK activity; in utero embryonic development; endopeptidase inhibitor activity"   <-> "activity"
# "activation of MAPK activity; in utero embryonic development; endopeptidase inhibitor activity"   <-> "utero"
# "activation of MAPK activity; in utero embryonic development; endopeptidase inhibitor activity"   <-> "endopeptidase; oxygen carrier activity; extracellular region; extracellular space; development"
# "carbonate" <-> "kidney development; morphogenesis of an epithelium; arylesterase activity; carbonate dehydratase activity"
# "kidney" <-> "kidney development; morphogenesis of an epithelium; arylesterase activity; carbonate dehydratase activity"
# "kidney" <-> "kidney development; morphogenesis of an epithelium; arylesterase activity; carbonate dehydratase activity"
# "kid" <-> "ney"
# "kid" <-> "arylesterase; kid"
# "development" <-> "kidney development; morphogenesis of an epithelium; arylesterase activity; carbonate dehydratase activity"
# "development" <-> "endopeptidase; oxygen carrier activity; extracellular region; extracellular space; development"
```
...
getPseudomatches <- function(col1, col2){
    matches_col1 <- as.character(sapply(col2, function (x){
        ...
        trimmed_GO <- gsub("\\s+$","", x) # Remove trailing spaces
        trimmed_col1 <- lapply(col1, function (x) gsub("\\s+$","", x))
        delimited_matches <- sapply(unlist(strsplit(trimmed_GO, "; ")), function( x ) { grepl(format(x), tolower(trimmed_col1))})
...
test<-getPseudomatches(test1,test2)
test
```
```{r}
getPseudomatches <- function(col1, col2){
...
test <- getPseudomatches(functool4, functool4)
```
