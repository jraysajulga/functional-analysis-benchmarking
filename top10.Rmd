...
metaquantome expand \
    --mode f \
    --data_dir data/databases/gene_ontology/mqome \
    --int_file results/mqome/input/eggnog/eggnog_int.tab \
    --pep_colname_int peptide \
    --func_file results/mqome/input/eggnog/eggnog_func.tab \
    --pep_colname_func peptide \
    --func_colname gos \
    --ontology go \
    --samps results/mqome/input/eggnog/eggnog_samples.tab \
    --outfile results/mqome/output/eggnog/func_expanded.tab
    
metaquantome filter \
    --expand_file results/mqome/output/eggnog/func_expanded.tab \
    --mode f \
    --ontology go \
    --samps results/mqome/input/eggnog/eggnog_samples.tab \
    --min_peptides 1 \
    --min_pep_nsamp 1 \
    --min_children_non_leaf 1 \
    --min_child_nsamp 1 \
    --qthreshold 1 \
    --outfile results/mqome/output/eggnog/func_filtered.tab
    
metaquantome stat \
    --file results/mqome/output/eggnog/func_filtered.tab \
    --mode f \
    --ontology go \
    --samps results/mqome/input/eggnog/eggnog_samples.tab \
    --parametric True \
    --paired \
    --outfile results/mqome/output/eggnog/func_stat.tab
```
```{r}
eggnog_expanded <- read.delim("results/mqome/output/eggnog/func_expanded.tab", stringsAsFactors = FALSE) %>%
    mutate(log2ratio = log2((countWS + 1)/(countNS + 1))) %>%
    arrange(-log2ratio)
eggnog_filtered <- read.delim("results/mqome/output/eggnog/func_filtered.tab", stringsAsFactors = FALSE) %>%
    mutate(log2ratio = log2((countWS + 1)/(countNS + 1))) %>%
    arrange(-log2ratio)
```
```{r}
...
```
```{r}
eggnog_w_counts[grepl('^((arginine)|(aspartate)|(glutamate 5-)|(homoserine)) kinase activity', eggnog_w_counts$name),]
```
...
# samp1_col <- 'countNS'
# samp2_col <- 'countWS'
write_mqome_inputs(unipept_all, 'unipept', 'GO.term', 'GO.term', 'countNS', 'countWS')
```
```{bash}
metaquantome expand \
    --mode f \
    --data_dir data/databases/gene_ontology/mqome \
    --int_file results/mqome/input/unipept/unipept_int.tab \
    --pep_colname_int peptide \
    --func_file results/mqome/input/unipept/unipept_func.tab \
    --pep_colname_func peptide \
    --func_colname 'GO.term' \
    --ontology go \
    --samps results/mqome/input/unipept/unipept_samples.tab \
    --outfile results/mqome/output/unipept/func_expanded.tab
    
metaquantome filter \
    --expand_file results/mqome/output/unipept/func_expanded.tab \
    --mode f \
    --ontology go \
    --samps results/mqome/input/unipept/unipept_samples.tab \
    --min_peptides 1 \
    ...
    --min_child_nsamp 1 \
    --qthreshold 1 \
    --outfile results/mqome/output/unipept/func_filtered.tab
    
metaquantome stat \
    --file results/mqome/output/unipept/func_filtered.tab \
    --mode f \
    --ontology go \
    --samps results/mqome/input/unipept/unipept_samples.tab \
    --parametric True \
    --paired \
    --outfile results/mqome/output/unipept/func_stat.tab
```

```{r}
unipept_expanded <- read.delim("results/mqome/output/unipept/func_expanded.tab", stringsAsFactors = FALSE) %>%
    mutate(log2ratio = log2((countWS + 1)/(countNS + 1))) %>%
    arrange(-log2ratio)
...
unipept_all <- inner_join(unipeptNS, unipeptWS, by = c("GO.term", "Name")) %>%
    ...
    arrange(-log2ratio)
write.table(unipept_all, file="quant_results/unipept.tab", quote=FALSE, row.names=FALSE)
```
...
prophane_gos <- removeDuplicates(prophane$name)
...
get_rank_table <- function(name, tools){
    ...
    rank_table[,"Ranking"] <- rep(c('Rank', 'WS / NS (FC)'), n)
        ...
        output <- sapply(tool_topn, function(topn){
            ...
            if (length(rank) > 0){
                ...
                list(rank[[1]],paste(ifelse(is.null(term$countWS) && is.null(term$countNS),
                                       paste('(', round(as.numeric(term$log2ratio), digits=2), ')', sep=''),
