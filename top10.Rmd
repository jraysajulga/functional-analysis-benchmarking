...
getMatches <- function(col_gos1, col_gos2, name){
    paths <- data.frame(Source = character(),
                        ...
                        Group = character(),
                        Value = numeric())
    nodes <- sapply(1:110, function ( x ) { paste(rep(" ", x), collapse="") })
...
unipept_nodes_unique <- as.data.frame(sapply(1:length(unipept_nodes), function( index ) { 
    sapply(unipept_nodes[,index], function ( row ) { paste(row, paste(rep(" ",110 * (index - 1)), collapse=""), sep="")})
}))
colnames(unipept_nodes_unique) <- colnames(unipept_nodes)
