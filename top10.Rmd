...
```{r}
for (source in unipept_paths_final$source){
    print(source)
    paths <- links[links$source == source,] %>% filter (group != "invisible")
    print(paths)
    print(sum(paths$value))
    links[links$source == source & links$group == "invisible",]$value <- links[links$source == source & links$group == "invisible",]$value - sum(paths$value)
}
```
...
unipept_paths_final
for (source in unipept_paths_final$source){
    paths <- links[links$source == term,] %>% filter (group != "invisible")
    links[links$source == source & links$group == "invisible",]$value =
        links[links$source == source & links$group == "invisible",]$value - sum(paths$value)
}
for (target in unipept_paths_final$target){
    paths <- links[links$target == term,] %>% filter (group != "invisible")
    links[links$target == source & links$group == "invisible",]$value =
        links[links$target == target & links$group == "invisible",]$value - sum(paths$value)
}
````
