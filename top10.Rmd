...
megan <- collapse_by_go(megan, ',')  %>% mutate(corrWS = countWS + 1,
                          corrNS = countNS + 1,
...
prophane <- data.table(inner_join(prophane_NS, prophane_WS, by = c("members_identifier")) %>%
    filter(ns != "-", ws != "-") %>%
    mutate(log2ratio = log2(as.double(ws)/as.double(ns)),
           gos = gsub(" ","", paste(ns_go1, ws_go1, ns_go2, ws_go2, ns_go3, ws_go3, sep=","))) %>%
    filter(gos != "") %>%
    select(log2ratio, countWS = ws, countNS = ns, gos, ns_go1, ws_go1, ns_go2, ws_go2, ns_go3, ws_go3))
...
prophane <- prophane[ , list( gos = unlist( strsplit( gos , "," ) ) ) , by = list(log2ratio, countWS, countNS) ]
prophane$name <- Term(prophane$gos)

# Collapse GO terms based on fold change values.
prophane <- prophane %>% group_by_at(vars(log2ratio, countWS, countNS)) %>%
...
get_rank_table <- function(name, tools){
    ...
    for (i in 1:length(tools)){
        output <- sapply(tool_topn, function(topn){
            tool <- tools[i][[1]]
            rank <- which(grepl(topn, tool$name, fixed=TRUE))[[1]]
            match <- tool[grepl(topn, tool$name, fixed=TRUE)[[1]],]
