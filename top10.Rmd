...
```{r}
unipept <- read.csv(file = "unipept_results/737ALL_Galaxy3-[Unipept_pept2go_on_data_1_tsv].tabular", sep="\t",
                    header = TRUE, stringsAsFactors = FALSE)

# Write peptides out for feeding into metaGOmics and BLAST
unipeptides <- unipept[grepl("peptide deformylase activity", unipept$go_names),]$X.peptide
for (peptide in unipeptides) {
    write.table(paste(peptide, "10", sep="\t"),
                file= paste("peptide_deformylase_peptides_unipept/",peptide, ".tab", sep=""), 
                quote = FALSE, row.names = FALSE, col.names = FALSE) 
}
```
```{r}
