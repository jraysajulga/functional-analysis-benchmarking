...
#functool5[grepl("\\^$", functool5)]
#functool3[grepl("\\^$", functool3)]
#functool3[29]
#functool3[29] %in% functool5
#functool3

getMatches <- function(col1, col2){
    matches_col1 <- as.character(sapply(col2, function (x){
        ifelse(gsub("\\s+$","", x) %in% lapply(col1, function (x) gsub("\\s+$","", x)),
        col1[gsub("\\s+$","", x) == lapply(col1, function (x) gsub("\\s+$","", x))],
       FALSE)
    }))
...
matches <- getMatches(functool3, functool5)
matches
```
...
values[(length(functool1) * 4 + 1):(length(functool1) * 4 + nrow(matches))] <- .1
#values <- "1"

# Make a connection data frame
links=data.frame(
    source=c(as.character(functool1), as.character(functool2), as.character(functool3), as.character(functool4), as.character(matches$source)),
    target=c(as.character(functool2), as.character(functool3), as.character(functool4), as.character(functool5), as.character(matches$target)),
    value= c(values))
# A3, D2, .5
...
link_groups <- c()
link_groups[1:(nrow(links)-nrow(matches))] <- "type_a"
link_groups[(nrow(links)-nrow(matches)+1):nrow(links)] <- "type_b"
#link_groups[1:nrow(links)] <- "type_a"
...
my_color <- 'd3.scaleOrdinal() .domain(["type_a", "type_b", "a", "b"]) .range(["white", "blue", "grey", "blue"])'
