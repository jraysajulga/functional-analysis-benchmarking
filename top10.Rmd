...
getMatches <- function(col_gos1, col_gos2, name){
    paths <- data.frame(Source = character(),
                        ...
                        Group = character(),
                        Value = numeric())
    nodes <- sapply(11:110, function ( x ) { paste(rep(" ", x), collapse="") })
    ...
    matches <- as.data.frame(unlist(sapply(col_gos1, function ( gos1 ){
        ...
        perc_list <- as.character(sapply(which(ranks != ""),
                                               function ( rank ){ ceiling(rank / length(col_gos2) * 100) }))
        if (length(rank_list) != 0) {
            ...
            path <- consolidator %>% mutate(Source = gos1,
                                            Group = name,
                                            Value = str_count(rank_list, "/") + 1) %>%
                select(Source, Target = perc_list, Value, Group)
            paths <<- rbind(paths, path)
            
            # Set up node information
            nodes[as.numeric(levels(perc_list))[perc_list]] <<- as.character(rank_list)
...
tool1 <- getMatches(unipept_topn, eggnog_w_counts$name[1:200], "EggNOG")
tool2 <- getMatches(unipept_topn, megan$name[1:200], "MEGAN")
tool3 <- getMatches(unipept_topn, metagomics_filt$name[1:200], "MetaGOmics")
tool4 <- getMatches(unipept_topn, pAll$name[1:200], "MPA")
tool5 <- getMatches(unipept_topn, prophane$names[1:200], "Prophane")
...
unipept_paths <- rbind(tool1$paths,
                       ...
                       tool4$paths,
                       tool5$paths)
unipept_nodes <- data.frame("EggNOG" = tool1$nodes,
                            "MEGAN" = tool2$nodes,
                            "MetaGOmics" = tool3$nodes,
                            "MPA" = tool4$nodes,
                            "Prophane" = tool5$nodes)
unipept_top_matches
```
```{r}
which(colnames(unipept_nodes) == "MEGAN")
```

```{r}
# Make each node unique by adding spaces after each one
unipept_nodes_unique <- sapply(1:length(unipept_nodes), function( index ) { 
    sapply(unipept_nodes[,index], function ( row ) { paste(row, paste(rep(" ",110 * (index - 1)), collapse=""), sep="")})
})
#unipept_nodes_unique

# unipept_paths %>% mutate(Target = sapply(Target, function ( row ) {
#     paste("DLFJK",row)
# }))
unipept_paths_unique$Target <- sapply(1:nrow(unipept_paths), function ( x ) { 
    index <- which(colnames(unipept_nodes) == unipept_paths[x,]$Group)
    paste(unipept_paths[x,]$Target, paste(rep(" ",110 * (index - 1)), collapse=""), sep="") } )

# unipept_paths_unique <- unipept_paths %>% mutate(Target = sapply(Target, function ( row ) {
#                                                                paste(row,
#                                                                "HERE: ",
#                                                                # sapply(Group, function ( tool ) {
#                                                                #     paste(rep("x",
#                                                                #               2 * (which(colnames(unipept_nodes) == tool) - 1)
#                                                                #               ),
#                                                                #        sep="")
#                                                                #     }),
#                                                                sep="")
...
#unipept_paths_unique <- unipept_paths %>% mutate(Target = paste(Target, "YO: ", 
#                                                                sapply(Group, function ( tool ) { 
#                                                                    which(colnames(unipept_nodes) == tool)
                                               #                 })
                                                                #paste(rep("x",110 * (sapply(Group, function ( tool ) { 
                                                                #    which(colnames(unipept_nodes) == tool)
                                                                #}) - 1)), sep=""),
                                                          #      sep=""))
#colnames(unipept_nodes)
unipept_paths_unique
```
