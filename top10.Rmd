...
pAll$name <- as.character(sapply(pAll$Protein.Accession, function ( protid ) {
    goids <- strsplit(mpa_uniprot[mpa_uniprot$Entry == protid,]$Gene.ontology.IDs, "; ")
    sapply(goids, function ( go_list ) { 
        if (length(go_list) > 1){
            names <- Term(filterOntology(go_list, ontology))
            paste(names[!is.na(names)], collapse="; ")
        } else {
            ""
        }
    })
...
for (label in labels){
    ...
    sankey_nodes_final$Term <- c(tool_topn, 1:10)
    sankey_nodes_final <- sankey_nodes_final[,c(6,1,2,3,4,5)]
    label_df <- data.frame(t(c("Term", labels)))
