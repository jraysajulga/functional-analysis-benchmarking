...
get_rank_table <- function(tool_topn, tools, labels){
    rank_table <- data.frame(names = tool_topn)
    for (i in 1:length(tools)){
        #print(labels[i])
        rank_table[,labels[i]] <- sapply(tool_topn, function(topn){
            tool <- tools[i][[1]]
            rank <- which(grepl(topn, tool$name, fixed=TRUE))
            match <- tool[grepl(topn, tool$name, fixed=TRUE),]
            matches <- sapply(1:nrow(match), function(row){
                if (row){
                    if (!is.na(rank[row])){
                        rank[row]
                    }
    ...
    rank_table
    # rank_table[,c(1,7,2,3,4,5,6)]
}

# tool_topn
get_rank_table(unipept_all$name[1:10],
