...
```{r}
eggnog_w_counts[grepl("peptide deformylase activity", eggnog_w_counts$name),]
nrow(eggnog_w_counts[grepl("peptide deformylase activity", eggnog_w_counts$name),])
eggnog_w_counts[grepl("peptide deformylase activity", eggnog_w_counts$name),]$sumCountWS - 1
eggnog_w_counts[grepl("peptide deformylase activity", eggnog_w_counts$name),]$sumCountNS - 1
```
...
metagomics <- read.delim("metaGOmics_results/go_compare_149_150.txt",
    ...
    select(go = GO.acc,
           ...
           count_ns = Run.1.PSM.count,
           count_ws = Run.2.PSM.count) %>%
...
Get the number of peptides (non-unique) at a peptide deformylase activity and spectral counts
```{r}
metagomics_filt[metagomics_filt$name == "peptide deformylase activity",]
metagomics_filt[metagomics_filt$name == "peptide deformylase activity",]$count_ws
metagomics_filt[metagomics_filt$name == "peptide deformylase activity",]$count_ns
```
...
Get the number of peptides (non-unique) at a peptide deformylase activity and spectral counts
```{r}
unipept_all[unipept_all$Name == "peptide deformylase activity",]
unipept_all[unipept_all$Name == "peptide deformylase activity",]$lapCountWS - 1
unipept_all[unipept_all$Name == "peptide deformylase activity",]$lapCountNS - 1
```
...
for (label in labels){
    ...
    if (nrow(ambiguous_paths) > 0){
        for (index in 1:nrow(ambiguous_paths)){
            ...
            range <- paste(sort(unlist(sapply(paths$target, function ( x ) { strsplit(x, " / ")[[1]] } ))), collapse=", ")
