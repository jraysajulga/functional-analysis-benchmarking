...
```{r}
write_mqome_inputs <- function(name, header, col_gos, col_samp1, col_samp2) {
    int_df <- eggnog_w_counts[c(header, col_samp1, col_samp2)]
    names(int_df)[1] <- 'peptide'
    write.table(int_df,
                file=paste0("results/mqome/input/", name, "/", name, "_int.tab"),
                quote=FALSE, row.names=FALSE, sep="\t")
    func_df <- eggnog_w_counts[c(header, col_gos)]
    names(func_df)[1] <- 'peptide'
    write.table(func_df,
                file=paste0("results/mqome/input/", name, "/", name, "_func.tab"),
                quote=FALSE, row.names=FALSE, sep="\t")
}
write_mqome_inputs('eggnog', 'gos', 'gos', 'countNS', 'countWS')
```

Let's expand and filter using metquantome
```{bash}
metaquantome expand \
    --mode f \
