...
```{r}
write_mqome_inputs(metagomics, 'metagomics', 'go', 'go', 'countNS', 'countWS')
print_mqome_cmds("metagomics", "go.1")
```
```{r}
...
print_mqome_cmds("unipept", "'GO.term.1'")
```
```{r}
...
png("results/correlation/um_cor.png", res = 700, width = 3.5, height = 4, units = "in")
...
```{r}
um <- inner_join(unipept_filtered, metagomics_filtered, by = c("id" = "id"))
png("results/correlation/um_cor_filtered.png", res = 700, width = 3.5, height = 4, units = "in")
plot(log2ratio.x ~ log2ratio.y, data = um, pch = 20, cex = 0.3, xlim=c(-2.5,2.5), ylim=c(-2.5,2.5),
     xlab = "Unipept: Log2 Ratio",
     ylab = "MetaGOmics: Log2 Ratio")
mod <- lm(log2ratio.x ~ log2ratio.y, data = um)
abline(0, 1, col = "red", lty = 2)
...
tools <- list(remove_blanks(eggnog_filtered),
              remove_blanks(megan_filtered),
              remove_blanks(metagomics_filtered),
              remove_blanks(mpa_filtered),
              remove_blanks(prophane_filtered),
              remove_blanks(unipept_filtered))
...
generate_overlap_plot <- function(ontology){
    ...
    gos <- list('eggnog' = unique(filterOntology(eggnog_w_counts$gos, ontology)),
            'megan' = unique(filterOntology(megan$gos, ontology)),
            'metagomics' = unique(filterOntology(metagomics$go, ontology)),
            'mpa' = unique(filterOntology(pAll$gos, ontology)),
            'prophane' = filterOntology(prophane$gos, ontology),
            'unipept' = unique(filterOntology(unipept_all$GO.term, ontology)))
    ...
    for (i in 1:6){
        for (j in 1:6){
            jacslim[i, j] <- jaccard(unique(filterOntology(gos_filt[[i]], ontology)),
                                     unique(filterOntology(gos_filt[[j]], ontology)))
        }
    }
    ...
    ggplot(jac_melt_all)+
        ...
        labs(x = NULL, y = NULL)
    ggsave(paste0("results/overlap_plots/overlap-", ontology, "-filtered.png"), height = 5, width = 12, units = "in", dpi = 700)
}
