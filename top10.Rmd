...
getMatches <- function(col_gos1, col_gos2, name){
    matches <- as.data.frame(unlist(sapply(col_gos1, function ( gos1 ){
        ...
        rank_list <- paste(which(ranks != ""), collapse=", ")
        perc_list <- paste(as.character(sapply(which(ranks != ""),
                                               function ( rank ){ ceiling(rank / length(col_gos2) * 100) })),
                           collapse=", ")
        if (rank_list != "") {
            paste(rank_list, " (", perc_list, ")", sep="")
        } else {
            ""
        }
    })))
...
unipept_top_matches <- plyr::join_all(
    list(getMatches(unipept_topn, eggnog_w_counts$name, "EggNOG"),
         getMatches(unipept_topn, megan$name, "MEGAN"),
         getMatches(unipept_topn, metagomics_filt$name, "MetaGOmics"),
         getMatches(unipept_topn, pAll$name, "MPA"),
         getMatches(unipept_topn, prophane$names, "Prophane")),
    by="name")
unipept_top_matches
```
