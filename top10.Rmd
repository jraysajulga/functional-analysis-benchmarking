...
getMatch <- function(gos1, gos2){
    delimiter <- "; "
    if ((grepl(",", gos1) & !grepl(";", gos1)) ||
        (grepl(",", gos2) & !grepl(";", gos2))){
        delimiter <- ","
    }
...
```{r}
#eggnog_gos <- eggnog_w_counts$name
#eggnog_all_gos <- unlist(str_split(eggnog_gos[1:10], "; "))
# eggnog_uniques <- eggnog_all_gos[!eggnog_all_gos %in% unique(eggnog_all_gos[duplicated(eggnog_all_gos)])]
# eggnog_uniques
# #eggnog_all_gos
# sapply(str_split(eggnog_gos[1:10], "; "), function ( go_list ) { paste(go_list[go_list %in% eggnog_uniques], collapse="; ")})
...
removeDuplicates <- function(go_col){
    ...
    sapply(str_split(eggnog_gos[1:10], "; "), function ( go_list ) {
        paste(go_list[go_list %in% eggnog_uniques], collapse="; ")})
}
...
```{r}
unipept_topn <- c("acetolactate synthase activity", # COG0028
                  "2-isopropylmalate synthase activity", #COG0119
                  "beta-N-acetylhexosaminidase activity", # ec 3.2.1.52
                  "cysteine-type endopeptidase activity; cysteine-type peptidase activity", # COG3579 aminopeptidase c
                  "ethanolamine kinase activity", # COG0510
                  "choline-phosphate cytidylyltransferase activity; cytidylyltransferase activity", # COG4750
                  "alcohol dehydrogenase (NAD) activity", # COG1979
                  "6-phosphogluconolactonase activity", # COG2706 6-phosphogluconolactonase
                  "7,8-didemethyl-8-hydroxy-5-deazariboflavin synthase activity", # COG1060
                  "sulfuric ester hydrolase activity") # COG1368

tool1_label <- "EggNOG"
tool2_label <- "MetaGOmics"
tool3_label <- "MPA"
tool4_label <- "Prophane"
tool5_label <- "Unipept"

tool1 <- getMatches(unipept_topn, eggnog_w_counts$name, tool1_label)
tool2 <- getMatches(unipept_topn, metagomics_filt$name, tool2_label)
tool3 <- getMatches(unipept_topn, pAll$name, tool3_label)
tool4 <- getMatches(unipept_topn, prophane$names, tool4_label)
tool5 <- getMatches(unipept_topn, unipept_all$Name, tool5_label)
...
nodes=data.frame(name=c(as.character(framework$source),
                        as.character(unipept_nodes_final[,length(unipept_nodes_final)])))
...
my_color <- paste('d3.scaleOrdinal() .domain(["invisible", "block", "top", "',
                  ...
                  tool3_label, '", "',
                  tool4_label, '", "',
                  tool5_label, '"]) .range(["white", "black", "#ffe070", "red", "blue", "green", "orange", "purple"])', sep="")
