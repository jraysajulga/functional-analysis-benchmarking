...
```{r}
um <- inner_join(unipept_all, metagomics, by = "gos")
...
generate_overlap_plot <- function(ontology){
    ...
    # gos <- list('eggnog' = unique((eggnog_w_counts %>% filter(namespace %in% ontology))$gos),
    #         'megan' =  unique((megan %>% filter(namespace %in% ontology))$gos),
    #         'metagomics' =  unique((metagomics_filt %>% filter(namespace %in% ontology))$go),
    #         'mpa' =  unique((pAll %>% filter(namespace %in% ontology))$gos),
    #         'prophane' =  unique((prophane %>% filter(namespace %in% ontology))$gos),
    #         'unipept' =  unique((unipept_all  %>% filter(namespace %in% ontology))$GO.term))
    # 
