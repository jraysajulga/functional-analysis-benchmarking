...
```
```{r}
prophane_WS <- prophane_WS %>% select(members_identifier,
...
prophane_NS <- prophane_NS %>% select(members_identifier,
...
#prophane_NS
#prophane_WS
dat <- inner_join(prophane_NS, prophane_WS, by = c("members_identifier")) %>%
    filter(ns != "-", ws != "-") %>%
...
#paste(unique(strsplit(dat$gos[1:2],",")),  sep=",")
#sapply(strsplit(dat$gos[1:2], ","), function(x) paste(unique(x), collapse = ","))
#dat$gos[1:3]
dat$gos <- sapply(strsplit(dat$gos,","), function(x) {paste(unique(x[x != "-"]), collapse = ",")})
dat
#strsplit(dat$gos[1:3],",")
```
