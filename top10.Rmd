...
getMatches <- function(col_gos1, col_gos2){
    matches <- as.data.frame(unlist(sapply(col_gos1, function ( gos1 ){
        ...
        ranks <- paste(which(ranks != ""), collapse=", ")
        if (ranks == "") { ranks = 0 }
    })))
...
getMatches(unipept_topn, eggnog_w_counts$name[1:10])
unipept_top_matches <- c(getMatches(unipept_topn, eggnog_w_counts$name[1:10]),
                         #getMatches(unipept_topn, megan$og)
                         getMatches(unipept_topn, metagomics_filt$name[1:10]),
                         #getMatches(unipept_topn, pAll$Protein.Description)
                         getMatches(unipept_topn, prophane$names[1:10]))
```
