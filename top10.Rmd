...
pAll <- pAll %>% group_by(gos) %>%
    ...
    mutate(name = Term(gsub(" ", "", gos)),
           nsCount = SpecCount.x + 1, wsCount = SpecCount.y + 1,
...
# Removes both original and duplicates
removeDuplicates <- function(go_col){
    all_gos <- unlist(str_split(go_col, "; "))
    uniques <- all_gos[!all_gos %in% unique(all_gos[duplicated(all_gos)])]
    
    unique_col <- sapply(str_split(go_col, "; "), function ( go_list ) {
        paste(go_list[go_list %in% uniques], collapse="; ")})
...
for (label in labels){
    ...
    sankey_nodes_final <- as.data.frame(sapply(1:length(sankey_nodes), function( index ) { 
        sapply(sankey_nodes[,index], function ( row ) { paste(row, paste(rep(" ",20 * index), collapse=""), sep="")})
    }))
    colnames(sankey_nodes_final) <- colnames(sankey_nodes)
    sankey_nodes_final$Term <- c(tool_topn, paste(seq(0, 90, by=10), " - ", seq(10, 100, by=10), "%", sep=""))
    sankey_nodes_final <- sankey_nodes_final[,c(6,1,2,3,4,5)]
    ...
    sankey_paths_final$target <- sapply(1:nrow(sankey_paths), function ( x ) { 
        ...
        paste(sankey_paths[x,]$target, paste(rep(" ",20 * index), collapse=""), sep="") } )
    ...
    nodes <- nodes %>% mutate(group = c("header", rep(label, 10),
                                        rep("invisible", 10),
                                        labels[1], 
                                        ifelse(gsub(" ", "", as.character(name[(21 + 2):(30 + 2)])) == "", "top", labels[1]),
                                        ifelse(gsub(" ", "", as.character(name[(31 + 2):(40 + 2)])) == "", "invisible", labels[1]),
                                        labels[2], 
                                        ifelse(gsub(" ", "", as.character(name[(41 + 3):(50 + 3)])) == "", "top", labels[2]),
                                        ifelse(gsub(" ", "", as.character(name[(51 + 3):(60 + 3)])) == "", "invisible", labels[2]),
                                        labels[3],
                                        ifelse(gsub(" ", "", as.character(name[(61 + 4):(70 + 4)])) == "", "top", labels[3]),
                                        ifelse(gsub(" ", "", as.character(name[(71 + 4):(80 + 4)])) == "", "invisible", labels[3]),
                                        labels[4],
                                        ifelse(gsub(" ", "", as.character(name[(81 + 5):(90 + 5)])) == "", "top", labels[4]),
                                        ifelse(gsub(" ", "", as.character(name[(91 + 5):(100 + 5)])) == "", "invisible", labels[4]),
                                        labels[5],
                                        ifelse(gsub(" ", "", as.character(name[(101 + 6):(110 + 6)])) == "", "top", labels[5]),
                                        ifelse(gsub(" ", "", as.character(name[(111 + 6):(120 + 6)])) == "", "invisible", labels[5])))
...
ambiguous_paths <- unique(ambiguous_paths[,c(3,5)])

#paste(sankey_paths[sankey_paths$percent == ambiguous_path$percent &
               #sankey_paths$group == ambiguous_path$group,]$target
test1 <- data.frame(sankey_paths, stringsAsFactors = FALSE)
test2 <- data.frame(sankey_nodes, stringsAsFactors = FALSE)
for (index in 1:nrow(ambiguous_paths)){
    ...
    range <- paste(min(sapply(paths$target, function ( x ) { min(strsplit(x, " / ")[[1]]) } )),
                   "-", max(paths$target))
    #total_gos <- length(tools$go_list[[which(ambiguous_path$group == tools$label)]])
    #percent <- as.numeric(levels(ambiguous_path$percent))[ambiguous_path$percent] / 10
    #range <- paste(ceiling(total_gos * percent), "-", ceiling(total_gos * (percent - 0.1)))
    #sankey_paths[sankey_paths$percent == ambiguous_path$percent &
    #                 sankey_paths$group == ambiguous_path$group,]
    
    # Update path and node
    test1[sankey_paths$percent == ambiguous_path$percent &
                    sankey_paths$group == ambiguous_path$group,]$target <- range
    #print(test2[decile + 10, ambiguous_path$group])
    #print(range)
    decile <- as.numeric(levels(ambiguous_path$percent))[ambiguous_path$percent]
    #print(test2[decile + 10, ambiguous_path$group])
    test2[decile + 10, ambiguous_path$group] <- range
    #test2[19, ambiguous_path$group] <- "decile"
}
