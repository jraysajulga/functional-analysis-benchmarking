...
#prophane_NS
#prophane_WS
dat <- data.table(inner_join(prophane_NS, prophane_WS, by = c("members_identifier")) %>%
    filter(ns != "-", ws != "-") %>%
...
dat <- dat[ , list( gos = unlist( strsplit( gos , "," ) ) ) , by = fc ]
dat$names <- Term(dat$gos)
dat <- dat %>% group_by_at(vars(fc)) %>%
