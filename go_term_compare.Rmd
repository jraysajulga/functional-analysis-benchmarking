...
eggnog_gos_clean_mf <- filterOntology(eggnog_gos_clean, ontology)

length(eggnog$V2)
length(unique(eggnog$V2)) # Total number of terms
length(eggnog_gos_clean)
length(eggnog_gos_clean %>% unique())
length(eggnog_gos_clean_mf)
length(eggnog_gos_clean_mf %>% unique())
```
...
clean_write(eggnog_gos_clean_mf, file=eggnog_list_file)
```
...
megan_gos_clean_mf <- filterOntology(megan_gos_clean, ontology)
clean_write(megan_gos_clean_mf, file="go_lists/megan.tab")
megan
length(megan$og)
length(megan$og %>% unique)
length(megan_gos_clean)
length(megan_gos_clean %>% unique)
length(megan_gos_clean_mf)
length(megan_gos_clean_mf %>% unique)
```
...
metagomics_gos_clean_mf <- filterOntology(metagomics_gos_clean, ontology)
clean_write(metagomics_gos_clean_mf, file="go_lists/metagomics.tab")
...
length(metagomics_gos_clean_mf) # Number of translated GO terms
length(metagomics_gos_clean_mf %>% unique())
...
clean_write(protAll %>% unique(), file="mpa_results/all_proteins.tab")
...
mpa_gos_clean_mf <- filterOntology(mpa_gos_clean, ontology)

length(mpa_uniprot$Entry)
length(mpa_uniprot$Entry %>% unique())
length(mpa_gos_clean)
length(mpa_gos_clean %>% unique())
length(mpa_gos_clean_mf)
length(mpa_gos_clean_mf %>% unique())

clean_write(mpa_gos_clean_mf, file="go_lists/mpa.tab")
```
...
prophane_gos_clean_mf <- filterOntology(prophane_gos_clean, ontology)
clean_write(prophane_gos_clean_mf, file="go_lists/prophane.tab")
...
length(prophane_gos_clean_mf)
length(prophane_gos_clean_mf %>% unique())
```
...
unipept_gos_clean_mf <- filterOntology(unipept_gos_clean, ontology)
clean_write(unipept_gos_clean_mf, file="go_lists/unipept.tab")

unipept_all
length(unipept_gos_clean)
length(unipept_gos_clean %>% unique())
length(unipept_gos_clean_mf)
length(unipept_gos_clean_mf %>% unique())
```

## Euler Diagram of Results
```{r}
gos <- list('eggnog' = eggnog_gos_clean_mf,
            'megan' = megan_gos_clean_mf,
            'metagomics' = metagomics_gos_clean_mf,
            'mpa' = mpa_gos_clean_mf,
            'prophane' = prophane_gos_clean_mf,
            'unipept' = unipept_gos_clean_mf)
```
...
for (i in 1:6){
    ...
    if (exslim[i] > 0){
        print(setdiff(gos_slims[[i]], others))
    }
...
for (i in 1:6){
    for (j in 1:6){
        jacslim[i, j] <- jaccard(unique(gos_slims[[i]]), unique(gos_slims[[j]]))
    }
}
