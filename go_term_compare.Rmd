...
length(filterOntology(eggnog_gos_clean, "MF"))
length(filterOntology(eggnog_gos_clean, "MF") %>% unique())
```
...
for (i in 1:6){
    for (j in 1:6){
        jacslim[i, j] <- jaccard(filterOntology(unique(gos_slims[[i]]), ontology),
                                 filterOntology(unique(gos_slims[[j]]), ontology))
    }
}
