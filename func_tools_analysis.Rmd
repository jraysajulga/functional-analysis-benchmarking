...
```{r}
data.frame(gos = unipept_hits, name = Term(unipept_hits))
```

```{r}
unipept_hits_expanded
```
```{r}
unipept_hits_filtered[unipept_hits_filtered$countWS_n_samp_children != 0,]
```
```{r}
tools_hits_filtered <- list(eggnog_hits_filtered,
                            megan_hits_filtered,
                            metagomics_hits_filtered,
                            mpa_hits_filtered,
                            prophane_hits_filtered,
                            unipept_hits_filtered)
tools_hits_filtered
# sapply(tools_hits_filtered, function(tool){
#   print(tool[tool$countWS_n_samp_children != 0,])
# })
```
...
```{r}
tool <- 'unipept'
read.delim(paste0('results/rank_tables/normal/MF/', tool, '-rank_table.tabular')) #%>%
  # filter(GO.Term %in% c('biotin binding'))
```
```{r}
read.delim(paste0('results/rank_tables/expanded/MF/', tool, '-rank_table.tabular'))
```

```{r}
read.delim(paste0('results/rank_tables/filtered/MF/', tool, '-rank_table.tabular'))
```
