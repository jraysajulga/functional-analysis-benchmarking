...
get_rank_table <- function(name, tools, folder, ontology){
    ...
    while (length(unique(tool_gos[1:n])) < 5){
      n <- n + 1
    }
    tool_topn <- unique(tool_gos[1:n])
    n <- 5
    # tool_topn <- tool_topn[1:n]
    # n <- 5
    while (length(unique(tool_gos[(length(tool_gos) - n + 1):length(tool_gos)])) < 5){
      n <- n + 1
    }
    tool_bottomn <- unique(tool_gos[(length(tool_gos) - n + 1):length(tool_gos)])
