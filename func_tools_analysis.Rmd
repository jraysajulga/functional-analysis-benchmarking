...
get_rank_table <- function(name, tools, folder, ontology){
    ...
    # n <- 5
    # while (length(unique(tool_topn[(length(tool_topn) - n + 1):length(tool_topn)])) < 5){
    #   n <- n + 1
    # }
    #tool_bottomn <- tool_topn[(length(tool_topn) - n + 1):length(tool_topn)]
    tool_topn <- c(tool_topn,
                   tool_gos[(length(tool_gos) - n + 1):length(tool_gos)])
