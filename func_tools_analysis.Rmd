...
```{r}
tools_gos <- list(filterOntology(eggnog_w_counts_gos, ontology),
              filterOntology(megan_gos, ontology),
              filterOntology(metagomics_filt, ontology),
              filterOntology(pAll_gos, ontology),
              filterOntology(prophane_gos, ontology),
              filterOntology(unipept_all, ontology))
```
...
sapply(tools_gos, function(tool){
    hits = tool$gos == "GO:0034618"
    print(which(hits))
    length(hits[hits == TRUE])
})
min(total_hits)
min_hits_go <- average_ranks[total_hits == min(total_hits)]
min_hits_go <- "GO:0034618"
min_hits_go
```
...
format_for_amigo <- function(gos) { paste(strsplit(gos, ',')[[1]], collapse=" ")}
filterOntology <- function(gos, ontology){
 gos_mf <- gos[Ontology(gos) == ontology] 
 gos_mf[!is.na(gos_mf)]
}
```

```{r}
format_for_amigo <- function(go, df){
 group <- get_go_rows(go, df)
 gos <- strsplit(as.character(group$gos), ',')[[1]] 
 paste(filterOntology(gos, 'MF'), collapse=" ") 
}
```

```{r}
get_go_rows(min_hits_go, prophane)
```
```{r}
#Term(strsplit(get_go_rows(min_hits_go, megan)$gos, ',')[[1]])
```

```{r}
format_for_amigo(min_hits_go, eggnog_w_counts)
format_for_amigo(min_hits_go, megan)
format_for_amigo(min_hits_go, metagomics_filt)
paste(filterOntology(strsplit(get_go_rows(min_hits_go, pAll)$gos[[1]], '; ')[[1]], "MF"), collapse=" ")

format_for_amigo(min_hits_go, prophane)
format_for_amigo(min_hits_go, unipept_all)
```
...
unipept_pep_refiltered <- unipept_pep %>%
  filter(str_detect(go_mf_filtered, "GO:0034618"))
unique(unipept_pep_refiltered$go_mf_filtered)
```
...
ontology <- 'MF'
tools <- list(filterOntology(eggnog_w_counts, ontology),
              filterOntology(megan, ontology),
              filterOntology(metagomics_filt, ontology),
              filterOntology(pAll, ontology),
              filterOntology(prophane, ontology),
              filterOntology(unipept_all, ontology))
...
tools <- list(filterOntology(eggnog_filtered, ontology),
              filterOntology(megan_filtered, ontology),
              filterOntology(metagomics_filtered, ontology),
              filterOntology(mpa_filtered, ontology),
              filterOntology(prophane_filtered, ontology),
              filterOntology(unipept_filtered, ontology))
