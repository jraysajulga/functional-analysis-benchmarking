...
unipept_all <- full_join(unipeptNS, unipeptWS, by = c("GO.term", "Name")) %>%
...
prophane <- data.table(full_join(prophane_NS, prophane_WS, 
                                 by = c("members_identifier")) %>%
    mutate(gos = gsub(" ","", paste(ns_go1, ws_go1, ns_go2, 
                                    ws_go2, ns_go3, ws_go3, sep=",")),
           ns = replace(ns, ns=='-', 0),
           ws = replace(ws, ws=='-', 0)) %>%
    mutate_each(funs(replace(., which(is.na(.)), 0))) %>%
...
prophane_gos <- prophane_gos[prophane_gos$gos != 'NA',]
prophane
prophane_gos
```
...
```{r}
collapse_df <- function(df, mode){
  if (mode == 'func'){
    df %>% group_by(peptide) %>%
      summarise(n = n()) %>%
      select(peptide) %>%
      mutate('gos.1' = peptide)
  } else if (mode == 'int'){
    df %>% group_by(peptide) %>%
      summarise(n = n(),
                countNS = sum(countNS),
                countWS = sum(countWS)) %>%
      select(peptide, countNS, countWS)
  }
}
write_mqome_inputs <- function(df, name, header, col_gos, col_samp1, col_samp2, folder="full", fill=FALSE) {
  ...
  names(func_df) <- c('peptide', 'gos.1')
  if (name %in% c('eggnog', 'prophane')){
    int_df <- collapse_df(int_df, 'int')
    func_df <- collapse_df(func_df, 'func')
    print(int_df)
    print(func_df)
  }
  write.table(int_df,
...
```{r}
print_mqome_cmds <- function(name, func_colname, folder="full"){
...
import_mqome_output <- function(name, type){
  read.delim(paste0("results/mqome/output/", name, "/", name, "_func_", type, ".tab"),
             stringsAsFactors = FALSE) %>%
    rename(gos = id) %>%
    mutate(
...
go_target <- "GO:0042586" # peptide deformylase activity
go_target <- "GO:0046527" # glucosyltransferase activity
# go_target <- "GO:0008410" # CoA-transferase activity
...
metagomics_peps <- unique(strsplit(as.character(target_row$peptidesNS), ',')[[1]],
                   strsplit(as.character(target_row$peptidesWS), ',')[[1]])
peptide_regex <- paste(paste0('(', metagomics_peps, ')'), collapse="|")
...
```
```{r}
Term(as.character(metagomics_hits))
```
...
unipept_pep <- read.delim('data/tool_outputs/func_tools/unipept_results/737all-unipept-pept2go.tabular') %>%
...
unipept_peps <- as.character(unipept_pep_refiltered$X.peptide)
...
How's the overlap of peptides between these peptide-level tools?
```{r}
length(eggnog_peps)
```
```{r}
length(metagomics_peps)
```
```{r}
length(unipept_peps)
```
```{r}
length(intersect(eggnog_peps, intersect(metagomics_peps, unipept_peps)))
```
```{r}
total_peptides <- unique(c(eggnog_peps, metagomics_peps, unipept_peps))
length(total_peptides)
```
```{r}
#length(intersect(unipept_peps, twenty_peptides))
```
...
```{r}
target_name <- gsub(' ', '_', Term(go_target))
```

### Peptides
Select 20 random peptides from the total peptides available
```{r}
set.seed(20)
twenty_peptides <- sample(total_peptides, 20)
twenty_peptides
write.table(twenty_peptides, 
            file=paste0('data/tool_inputs/', target_name,
                        '_twenty_peps.tabular'),
            quote=FALSE, row.names=FALSE, col.names=FALSE)
```
...
write.table(format_to_fasta(total_peptides),
                   file=paste0('data/tool_inputs/blast2go/', target_name,'_peps.fasta'),
            quote=FALSE, row.names=FALSE, col.names=FALSE)
```
...
```{r}
# write.table(data.frame(total_peptides,1),
#             file=paste0('data/tool_inputs/metagomics/', target_name, '_peps.txt'), sep="\t",
#             quote=FALSE, row.names=FALSE, col.names=FALSE)
```

<!-- ### EggNOG -->
<!-- ```{r} -->
<!-- write.table(uni_glucosyl_peps, -->
<!--             file=paste0('data/tool_inputs/eggnog/', target_name, '_peps.tabular'), -->
<!--             quote=FALSE, row.names=FALSE, col.names=FALSE) -->
<!-- # eggnog %>% filter(peptide %in% uni_glucosyl_peps) -->
<!-- ``` -->
...
get_exclusive <- function(gos){
  ...
  sapply(all, function(i){
    ...
    go_ex <- setdiff(gos[[i]], others)
    if (length(go_ex) == 1){
      print(Term(go_ex))
    }
    format(length(go_ex), big.mark=",")
  })
}
create_summary_df <- function(ontology){
  ...
  summary_df <- data.frame(
    rbind(c('Proteins', 'eggNOG orthologous groups', 'GO terms',
          ...
          sapply(tools, function(tool) {format(nrow(tool), big.mark=",")}),
          sapply(tools_gos, function(tool) {
            paste0(format(length(tool$gos), big.mark=","), ' (', format(length(unique(tool$gos)), big.mark=","), ')')
          }),
          sapply(tools_gos, function(tool) {
            gos <- filter_ontology_df(tool, ontology)$gos
            paste0(format(length(gos), big.mark=","), ' (', format(length(unique(gos)), big.mark=","), ')')
          }),
          get_exclusive(sapply(tools_gos, function(tool){
            filter_ontology_df(tool, ontology)$gos
          })),
          sapply(tools_expanded, function(tool){
            format(length(filter_ontology_df(tool, ontology)$gos), big.mark=",")
          }),
...
get_rank_table <- function(name, tools, folder, ontology){
    ...
    # }
    labels <- c("EggNOG", "MEGAN", "MetaGOmics", "MPA", "Prophane", "Unipept")
    tool_gos <- filterOntology(expand_gos(tools[which(labels == name)][[1]]$gos), ontology)
    tool_gos <- tool_gos[tool_gos != "GO:0003674"] # filter out molecular_function
    tool_gos <- tool_gos[!is.na(tool_gos)]
    i <- n
    while (length(unique(tool_gos[1:i])) < n){
      i <- i + 1
    }
    tool_topn <- unique(tool_gos[1:i])
    i <- n
    # tool_topn <- tool_topn[1:n]
    # n <- 5
    while (length(unique(tool_gos[(length(tool_gos) - i + 1):length(tool_gos)])) < n){
      i <- i + 1
    }
    tool_bottomn <- unique(tool_gos[(length(tool_gos) - i + 1):length(tool_gos)])
    tool_topn <- c(tool_topn,
                   tool_bottomn)
        ...
        output <- sapply(1:length(tool_topn), function(j){
            ...
            if (length(ranks) > 0){
                ...
                list(paste(ifelse(is.null(term$countWS) && is.null(term$countNS),
                                       paste0('(', round(as.numeric(term$log2ratio),
                                                         digits=2), ')'),
                                       paste0(round(as.numeric(term$log2ratio),
                                                    digits=2),
                                              ' (', round(as.numeric(term$countWS),
                                                          digits=2), ' / ',
                                                    round(as.numeric(term$countNS),
                                                          digits=2), ')'))),
