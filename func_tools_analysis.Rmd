...
#### Chosen GO term
```{r}
go_target <- 'GO:0016705' # longest character term
Term(go_target)
```
...
```{r}
rows <- get_go_rows(min_hits_go, eggnog_w_counts)
eggnog_min_hits <- filterOntology(split_gos(rows$gos), 'MF')
rows
eggnog_min_hits
```
```{r}
Term(eggnog_min_hits)
```
```{r}
Term(megan_min_hits)
```
...
```{r}
rows <- get_go_rows(min_hits_go, megan)
megan_min_hits <- filterOntology(split_gos(rows$gos), 'MF')
rows
megan_min_hits
```
...
```{r}
rows <- get_go_rows(min_hits_go, pAll)
...
```{r}
rows <- get_go_rows(min_hits_go, prophane)
prophane_min_hits <- filterOntology(split_gos(unique(rows$gos)), 'MF')
rows
prophane_min_hits
```
