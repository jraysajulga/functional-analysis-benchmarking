...
```{r}
#rows$gos
filterOntology(as.character(split_gos(rows$gos, delimiter='; ')), 'MF')
```

Metagomics
```{r}
rows <- get_go_rows(min_hits_go, metagomics_filt)
metagomics_min_hits <- filterOntology(split_gos(rows$gos, delimiter='; '), 'MF')
rows
metagomics_min_hits
```
...
```
```{r}
unipept_pep[grepl("GO:0033727", unipept_pep$go_terms),]
```
