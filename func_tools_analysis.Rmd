...
tools_mqome <- list(filter_to_nodes(eggnog_expanded), 
...
tools_mqome <- list(filter_to_ancestors(eggnog_expanded), 
...
tools_mqome <- list(eggnog_expanded, 
                    megan_expanded,
                    metagomics_expanded,
                    mpa_expanded,
                    prophane_expanded,
...
# tools_mqome <- list(eggnog_filtered, 
#                     megan_filtered,
#                     metagomics_filtered,
#                     mpa_filtered,
#                     prophane_filtered,
...
metagomics_min_hits <- target_rows$gos
target_rows
paste(metagomics_min_hits, collapse=" ")
```
...
```{r}
eggnog_peptide <- eggnog[grepl(go_target, eggnog$gos),]$peptide
target_peptides # Metagomics
eggnog_peptide
unipept_pep$X.peptide
```
```{r}
...
find_pep_in_tool <- function(peptide, labels){
  ...
  paste(labels[sapply(labels, function(label) {"YVGDAIALVATTEKK" %in% target_peps[[label]]})],
      collapse=",")
}
...
create_summary_df <- function(ontology){
  ...
  headers <- list('Type of functional annotation',
                  ...
                  'Total (or unique) number of translated GO terms for all ontologies',
                  paste0('Total (or unique) number of translated ', translations[ontology], ' GO terms'),
  ...
  summary_df <- data.frame(
    rbind(c('Proteins', 'eggNOG orthologous groups', 'GO terms',
          ...
          sapply(tools_mqome, function(tool){
            length(filter_ontology_df(tool, ontology)$gos)
          }),
          get_exclusive(sapply(tools_mqome, function(tool){
            filter_ontology_df(tool, ontology)$gos
          }))
    )
  )
...
generate_overlap_plot <- function(ontology){
  ...
  gos <- sapply(tools_gos, function(tool) {filter_ontology_df(tool, ontology)$gos})
  ...
  gos_filt <- sapply(tools_mqome, function(tool){ filter_ontology_df(tool, ontology)$gos })
  ...
  print(ontology)
  print(labels)
  print(sapply(tools, function(tool){ nrow(tool)}))
  print(sapply(tools_mqome, function(tool){ nrow(tool)}))
...
get_rank_table <- function(name, tools, folder, ontology){
    ...
    rank_table <- data.frame(names = Term(unlist(lapply(tool_topn, function(a) { c(as.character(a), '')}))))
    rank_table[,"Ranking"] <- rep(c('FC (WS / NS)', 'Percentile (%)'), n)
    for (i in 1:length(tools)){
        output <- sapply(tool_topn, function(topn){
            tool <- tools[i][[1]]
            rank <- which(grepl(topn, tool$gos, fixed=TRUE))
            if (length(rank) > 0){
                term <- tool[rank[[1]],]
    ...
    write.table(rank_table,
                file=paste0("results/rank_tables/", ontology, '/', folder, "/", name, "-rank_table.tabular"),
                sep='\t', quote=FALSE, row.names=FALSE)
    rank_table
}
rank_tables <- sapply(c('MF', 'BP', 'CC', 'all'), function(ontology){
  list(get_rank_table("Unipept", tools, 'full', ontology),
       get_rank_table("EggNOG", tools, 'full', ontology),
       get_rank_table("MEGAN", tools, 'full', ontology),
       get_rank_table("MetaGOmics", tools, 'full', ontology),
       get_rank_table("MPA", tools, 'full', ontology),
       get_rank_table("Prophane", tools, 'full', ontology),
       get_rank_table("Unipept", tools_mqome, 'filtered', ontology),
       get_rank_table("EggNOG", tools_mqome, 'filtered', ontology),
       get_rank_table("MEGAN", tools_mqome, 'filtered', ontology),
       get_rank_table("MetaGOmics", tools_mqome, 'filtered', ontology),
       get_rank_table("MPA", tools_mqome, 'filtered', ontology),
       get_rank_table("Prophane", tools, 'filtered', ontology))
})
```
```{r}
read.delim('results/rank_tables/MF/full/mpa-rank_table.tabular')
read.delim('results/rank_tables/MF/filtered/mpa-rank_table.tabular')
```
...
for (i in 1:length(tools)){
  ...
  write.table(tools[[i]]$gos, file=paste0('results/go_output/go_only/filtered/',
                                          tolower(labels[i]), '.tabular'),
              row.names=FALSE, col.names=FALSE, quote=FALSE)
  write.table(filter_ontology_df(tools[[i]], "MF")$gos, file=paste0('results/go_output/go_only/MF/',
                                          tolower(labels[i]), '.tabular'),
              row.names=FALSE, col.names=FALSE, quote=FALSE)
}
```
